...
class Scheduler:
    def __init__(self):
        ...
        try:
            ...
            log.exception(dmsg('') + 'Error during scheduler init :(  --> ' + e.__str__())

    def scheduler_listener(self, event):
        if event.exception:
            print('The job crashed :(')
            log.debug(dmsg('') + 'The job crashed :(')
        else:
            log.debug(dmsg('') + 'The job has finished :-)')
    ...
    def add_cron_job(self, function, kwargs):
        ...
        try:
            ...
            log.exception(dmsg('') + 'Could not add job to scheduler :(  --> ' + e.__str__())
            return None

    def add__job(self, function, kwargs):
        try:
            # max_instances = 1, coalesce = True, misfire_grace_time = 2 ** 30,
            job = self.sched.add_job(replace_existing=True, func=function, trigger='interval', **kwargs)
            return job
        except Exception as e:
            log.exception(dmsg('') + 'Could not add job to scheduler :(  --> ' + e.__str__())
            return None
